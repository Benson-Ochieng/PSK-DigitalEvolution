...
export function getExpressShippingStatus(date: Date = new Date()): {
  ...
  const day = nairobiDate.getDay(); // 0 = Sunday, 1 = Mon, ..., 6 = Sat
  const hour = nairobiDate.getHours();
  const minute = nairobiDate.getMinutes();
  const timeInMinutes = hour * 60 + minute;

  const month = nairobiDate.getMonth() + 1; // 1-12
  const dayOfMonth = nairobiDate.getDate();

  // Kenyan Public Holidays
  const isPublicHoliday = (
    (month === 1 && dayOfMonth === 1) ||  // New Year's Day
    (month === 5 && dayOfMonth === 1) ||  // Labour Day
    (month === 6 && dayOfMonth === 1) ||  // Madaraka Day
    (month === 10 && dayOfMonth === 10) || // Mazingira Day / Utamaduni
    (month === 10 && dayOfMonth === 20) || // Mashujaa Day
    (month === 12 && dayOfMonth === 12) || // Jamhuri Day
  ...
  if (day === 6) {
    ...
    if (timeInMinutes >= 8 * 60 && timeInMinutes < 11 * 60) {
      return {
        isAvailable: true,
        statusText: "Available today until 11:00 AM (2hr turnaround)"
      };
    }
    return {
      isAvailable: false,
  ...
  // Weekdays (Mon - Fri): 8:00 AM to 3:00 PM (15:00)
  if (timeInMinutes >= 8 * 60 && timeInMinutes < 15 * 60) {
    return {
      isAvailable: true,
      statusText: "Available today until 3:00 PM (2hr turnaround)"
    };
  }
...
export default function CheckoutPage() {
  ...
  if (isAllDonation) {
    ...
    if (selectedCity === "Nairobi") {
      if (finalShippingMethod === "express") {
        deliveryFee = 500;
        deliveryFeeLabel = "Express Shipping(3hr)";
      } else {
        deliveryFee = 0;
  ...
  const isExpressEligible = !hasFreeShippingProduct && isNeighbourhoodProvided && isNeighbourhoodExpressAllowed && isExpressTimeAvailable;

  useEffect(() => {
    if (!isExpressEligible && shippingMethod === "express") {
      setShippingMethod("standard");
    }
  }, [isExpressEligible, shippingMethod]);

  useEffect(() => {
    const targetEmail = (customerEmail || recipientEmail || "").trim();
    const targetPhone = (customerPhone || recipientPhone || "").trim();
    if (!targetEmail && !targetPhone) return;

    const timer = setTimeout(async () => {
      try {
        const res = await fetch(`/api/loyalty?email=${encodeURIComponent(targetEmail)}&phone=${encodeURIComponent(targetPhone)}`);
        const data = await res.json();
        if (data.success && data.loyalty) {
          setLoyaltyState(data.loyalty);
        }
      } catch (err) {
        console.error("Error fetching loyalty points at checkout:", err);
      }
    }, 400);

    return () => clearTimeout(timer);
  }, [customerEmail, customerPhone, recipientEmail, recipientPhone]);
  ...
  const handleApplyLoyaltyPoints = () => {
    ...
    if (available <= 0) {
      setLoyaltyMessage("No loyalty points available in your balance yet. Points will be earned automatically on this purchase!");
      return;
    }
    if (usable <= 0) {
    ...
    if (!isAllDonation && isNeighbourhoodProvided) {
      ...
      if (selectedCity === "Nairobi") {
        if (finalShippingMethod === "express") {
          ...
          updatedDeliveryFeeLabel = "Express Shipping(3hr)";
        } else {
        ...
        <div style={{ maxWidth: "1200px", margin: "0 auto", padding: "0 1.5rem" }}>
          ...
          {successOrderNumber ? (
            ...
            <>
              ...
              {(() => {
                const nDate = new Date(new Date().toLocaleString("en-US", { timeZone: "Africa/Nairobi" }));
                const currentMonth = nDate.getMonth() + 1;
              ...
              <form className="checkout-form" onSubmit={handlePlaceOrder}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem" }}>
                  ...
                  <div style={{
                    ...
                    {selectedCity === "Nairobi" && selectedZone && selectedZone !== "Select your Neighbourhood" && isNeighbourhoodExpressAllowed ? (
                      isExpressTimeAvailable ? (
                        /* Image 3: Nairobi + eligible neighbourhood selected AND active during 8am - 3pm */
                        <div style={{
                          display: "flex",
                          justifyContent: "space-between",
                          ...
                          <span style={{ fontWeight: "bold", fontSize: "0.9rem", color: "#000000" }}>
                            Delivery Options
                          </span>
                          <div style={{ display: "flex", flexDirection: "column", gap: "0.65rem", alignItems: "flex-start" }}>
                            <label style={{ display: "flex", alignItems: "center", gap: "8px", cursor: "pointer", fontSize: "0.88rem", fontWeight: "bold", color: "#000000", margin: 0 }}>
                              <input
                                type="radio"
                            ...
                            <label style={{ display: "flex", alignItems: "flex-start", gap: "8px", cursor: "pointer", fontSize: "0.88rem", fontWeight: "bold", color: "#000000", margin: 0 }}>
                              <input
                                type="radio"
                                name="delivery_option"
                                value="express"
                              ...
                              <div style={{ display: "flex", flexDirection: "column" }}>
                                ...
                                <span style={{ fontSize: "0.78rem", fontWeight: "bold", color: "#000000" }}>8am -3pm</span>
                              </div>
                            </label>
                          </div>
                        </div>
                      ) : (
                        /* Nairobi + eligible neighbourhood selected, but time has elapsed / outside hours */
                        <div style={{
                          display: "flex",
                          justifyContent: "space-between",
                          alignItems: "flex-start",
                          ...
                          <span style={{ fontWeight: "bold", fontSize: "0.9rem", color: "#000000", minWidth: "120px" }}>
                            Delivery Options
                          </span>
                          <div style={{ fontSize: "0.85rem", fontWeight: "bold", color: "#000000", lineHeight: "1.45", textAlign: "left", maxWidth: "420px" }}>
                  ...
                  <div style={{
                    ...
                    {customerEmail || recipientEmail || recipientPhone ? (
                      <>
                        <div style={{ color: "#515151", fontSize: "0.8rem", lineHeight: 1.4, marginTop: "0.6rem", marginBottom: "0.75rem" }}>
                          ...
                          <span style={{ display: "block", color: "#64748b", fontSize: "0.75rem", marginTop: "0.2rem" }}>
                            (You earn loyalty points automatically on every purchase!)
                          </span>
                        </div>
                        ...
                        {Number(loyaltyState?.balance || 0) > 0 ? (
                          <div style={{ display: "flex", gap: "0.5rem" }}>
                            <input
                              type="number"
                              min="0"
                          ...
                          <div style={{ color: "#64748b", fontSize: "0.8rem", fontStyle: "italic", marginTop: "0.4rem" }}>
                            Completing this order will automatically credit loyalty points to your account!
                          </div>
                        )}
                      </>
                    ) : (
