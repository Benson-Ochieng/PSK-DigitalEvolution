...
export async function action({ request }: { request: Request }) {
  ...
  if (intent === "add_post" || intent === "save_post_details") {
    ...
    return redirect("/vp-backend/posts?view=all");
  }
...
export default function VpBackendPosts() {
  ...
  const handleTrash = async (id: string) => {
    ...
    body.append("id", id);
    await fetch(window.location.href, { method: "POST", body });
    window.location.href = "/vp-backend/posts?view=all";
  };
  ...
  return (
    <div className="posts-page">
      ...
      <div className="posts-tabs">
        <a href="/vp-backend/posts?view=all" className={`tab-btn ${currentView === "all" ? "active" : ""}`}>
          📋 All Posts
        </a>
        <a href="/vp-backend/posts?view=new" className={`tab-btn ${currentView === "new" || currentView === "edit" ? "active" : ""}`}>
          ➕ Add Post
        </a>
        <a href="/vp-backend/posts?view=categories" className={`tab-btn ${currentView === "categories" ? "active" : ""}`}>
          📂 Categories
        </a>
        <a href="/vp-backend/posts?view=tags" className={`tab-btn ${currentView === "tags" ? "active" : ""}`}>
          🏷️ Tags
        </a>
      </div>
                    ...
                    <tr key={p.id}>
                      <td style={{ padding: viewMode === "extended" ? "18px 20px" : "12px 20px" }}>
                        ...
                        <div className="row-actions">
                          <a href={`/vp-backend/posts?view=edit&id=${p.id}`} className="action-link">Edit</a>
                          <span>|</span>
                          <button
                            type="button"
        ...
        <Form method="post" className="editor-form">
          ...
          <div className="form-row">
            ...
            <input
              ...
              placeholder="e.g. https://visionplus.co.ke/my-post (leave blank to use local post slug)"
              className="form-input"
            />
          </div>
          ...
          <div style={{ display: "flex", gap: "12px", marginTop: "10px", alignItems: "center" }}>
            ...
            <a href="/vp-backend/posts?view=all" className="btn-secondary" style={{ textDecoration: "none", textAlign: "center" }}>
              Cancel
            </a>
          </div>
        </Form>
